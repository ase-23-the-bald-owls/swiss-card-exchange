...
CREATE TABLE
  addresses (
    ...
    city TEXT NOT NULL,
    creator_expiry_time TEXT DEFAULT (auth.jwt () ->> 'exp')::TEXT
  );
...
CREATE POLICY "users and creators of customers can see their customers." ON customer FOR
SELECT
  USING (
    ((auth.jwt () ->> 'sub')::UUID = customer.user_id)
    OR (
      (auth.jwt () ->> 'exp')::TEXT = creator_expiry_time
    )
  );

-- row level security for addresses, needs table customer
...
CREATE POLICY "users and creators or addresses can see their addresses." ON addresses FOR
SELECT
  USING (
    (
      (auth.jwt () ->> 'sub')::UUID IN (
        SELECT
          c.user_id
        FROM
          customer c
        WHERE
          c.shipping_address_id = addresses.id
          OR c.billing_address_id = addresses.id
      )
    )
    OR (
      (auth.jwt () ->> 'exp')::TEXT = creator_expiry_time
...
-- end row level security for addresses
--
CREATE TABLE
  orders (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE POLICY "users and the creators of orders can see their orders." ON orders FOR
SELECT
  USING (
    (auth.jwt () ->> 'sub')::UUID IN (
      SELECT
    ...
    OR (auth.jwt () ->> 'exp')::TEXT = creator_expiry_time
  );
...
CREATE POLICY "users and creator of orderitems can see their orderitems." ON orderitem FOR
SELECT
  USING (
    (auth.jwt () ->> 'sub')::UUID IN (
      SELECT
